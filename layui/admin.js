    ...
    var a = layui.jquery,
        ...
        F = P.events = {
            ...
            message: function (e) {
                ...
                // console.log(e.find("#messageDo"))
                // e.find("#messageDo").hide();
                $.ajax({//获取未读消息数
            ...
            messageToPage:function(){//自定义刷新跳转页方法
                var e = ".layadmin-iframe",
                hrefStr = a(this).attr("lay-href");
                                ...
                                'success': function (data) {
                                    if(data.status_code==200){
                                        if(data.data.total_count==0){
                                            ...
                                            // console.log($("#messageDo" , parent.document),data.data.total_count)
                                            $("#messageDo" , parent.document).html(data.data.total_count);
                                            $("#messageDo" , parent.document).removeClass('layui-hide');
                                        }
                                        $(parent.document).find(".layadmin-iframe").each(function(){
                                            if($(this).attr("src")=='company/PersonalAtten/MessageCenter.html'){
                                                console.log($(this)[0].contentWindow.layui.table)
                ...
                $(parent.document).find(".layadmin-iframe").each(function(){
                    if($(this).attr("src")==hrefStr){
                        $(this)[0].contentWindow.location.reload(!0);
                    }
                }) 
            }
        };
