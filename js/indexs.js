                        ...
                        if (data.status_code == 200) {
                            ...
                            if (data.data.msg.length > 0) {
                                ...
                                for (var i = 0; i < data.data.msg.length; i++) {
                                    htmls += '<li><a class="messageToPage" data-id="' + data.data.msg[i].message_id + '" layadmin-event="messageToPage" lay-text="消息中心" lay-href="' + data.data.msg[i].message_url + '?id=' + data.data.msg[i].message_content_id + '">' + data.data.msg[i].message_content + '来自' + data.data.msg[i].from_name + '</a></li>'
                                }
                                layer.open({
                                    type: 2,
                                    title: "新消息",
        ...
        $.menuInit = function (ev, language) {
            var menu = { //导航参数及方法
                ...
                openFrome: function (elem) {
                    ...
                    $("#LAY-system-side-menu").find("li").removeClass("openFr");
            ...
            $.each(menu.menuData, function (key, val) {
                if (key == "topNav") {
                    for (var i = 0; i < menu.menuData[key].length; i++) { // console.log(topNav);
                        menu.tNavHtml +=
                            '<li  class="layui-nav-item  menuT_' + menu.menuData[key][i].id +
                            '"><a href="javascript:void(0)" lay-tips="' + menu.menuData[key][i].display_name + '" ><i class="layui-iconFont iconfont ' + menu.menuData[key][i].icon + '"></i><cite>'
                ...
                if (key == "leftNav") {
                    for (var i = 0; i < menu.menuData[key].length; i++) { //顶部切换的             // console.log(topNav);
                        ...
                        for (var k = 0; k < menu.menuData[key][i].length; k++) {
                            ...
                            if (menu.menuData[key][i][k].children && menu.menuData[key][i][k].children.length > 0) { //包含子导航
                                ...
                                for (var j = 0; j < menu.menuData[key][i][k].children.length; j++) {
                                    if(menu.menuData[key][i][k].children[j].children){
                                        menu.lNavHtml += '<dd class=" parent_' + menu.menuData[key][i][k].parent_id + '_' + menu.menuData[key][i][k].children[j].id +'"><a>' + menu.menuData[key][i][k].children[j].display_name +'</a><dl class="layui-nav-child">'
                                        for(var l=0; l<menu.menuData[key][i][k].children[j].children.length;l++){
                                            menu.lNavHtml += '<dd class=" parent_' + menu.menuData[key][i][k].children[j].parent_id + '_' + menu.menuData[key][i][k].children[j].children[l].id +'"><a class="noChild"   lay-href="' + menu.menuData[key][i][k].children[j].children[l].static_html +'">' + menu.menuData[key][i][k].children[j].children[l].display_name +'</a></dd>'
                                        }
                                        menu.lNavHtml+='</dl></dd>'
                                    }else{
                                ...
                                menu.lNavHtml +=
                                    '<dd class=" parent_' + menu.menuData[key][i][k].parent_id + '"><a class="noChild"  lay-href="' +
                            ...
                            $(".menuT_" + menu.menuData[key][i][k].parent_id + "").children('dl').append(menu.lNavHtml);
                            menu.lNavHtml = "";
                        }
