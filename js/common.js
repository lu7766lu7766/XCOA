...
$.fetch = function (o = {}) {
    ...
    $.ajax({
        ...
        complete(){
            // console.log(o.url)
            ajaxSenquce.splice(ajaxSenquce.findIndex(v=>v==o.url),1);
        }
    })

}
var selMemberFun=function(el){//添加用户弹窗
    //el 当前点击的标签
    layTit=el.parent().attr('data-title');
    if(layTit == ''){
        layTit=' ';
    }
    var pophtml ='<div class=" popUpsCont popUps">'
    ...
    layer.open({
        ...
        yes: function(index, layero){
            //do something
            var nameArr='',idstr='',nameText='',titlename='',departmentName='';
            ...
            el.parents().find("#workTitle").val(departmentName);
            el.parents().find("#titlename").val(titlename)
            selTerr([],$("#browser2"),2);
            selTerr([],$("#browser3"),1);
            selTerr([],$("#browser1"),1);
            $("#userList>ul").html('');
    ...
    var selTerr =function(o,el,ls){//生成列表的方法
        var html ='';
        function forTree(o){
            for(var i =0;i<o.length;i++){
                html+='<li>'
                if(o[i].children&&o[i].children.length>0){
                    html+='<span class="isChid folder" data-id="'+o[i].id+'"><i class="iconfont bclick icon-40"></i><i class="iconfont icon-zy_qunzuduoren"></i><a data-id="'+o[i].id+'">'+o[i].name+'</a></span><ul>'
                    forTree(o[i].children);//存在子级再次调用循环
                    html+="</ul>"
                }else{
                    html+='<span class="noChid Shrink file" data-id="'+o[i].id+'"><i class="iconfont null"></i><i class="iconfont icon-zy_qunzuduoren"></i><a data-id="'+o[i].id+'">'+o[i].name+'</a></span>'
                }
        ...
        forTree(o);
        el.html(html);
        el.find("span.isChid>i.bclick").click(function(){//展示样式
        ...
        el.off("click").on("click","span>a",function(){  
            ...
            if(ls==1){
                ...
                $("#addAllDePUser").show().attr("data-nid",Id);
                $("#addAllRolUser").hide();
            }
            if(ls==2){
                var obj= {
                ...
                $("#addAllRolUser").show().attr("data-nid",Id);                                
                $("#addAllDePUser").hide();
            }
            $("#main1>ul.filetree").find("span").removeClass("on");
    ...
    var clickTerr =function(o={},isSq){//列表选择后执行的方法  
        // if(isSq&&o.userName==''){
        //     layer.msg('请输入用户名');
        //     return false
        // }
        $.fetch({
            ...
            cb:function(rs){
                ...
                if(!isSq){
                    $("#btmChosBtn").show();
                    $("#addAllDePUser").off("click").on("click",function(){
                        ...
                        $.fetch({
                            ...
                            cb:function(rs){
                                for(var i=0;i<rs.data_list.length;i++){
                                    if(roleList.indexOf(''+rs.data_list[i].id) == -1){//判断数组是否存在当前id
                                        roleList.push(''+rs.data_list[i].id);
                                        var ulist =  '<li class="pick" data-user="'+rs.data_list[i].id+'">'+rs.data_list[i].name+' ('+rs.data_list[i].title_name+')</li>'
                                        $("#pickUserlist>ul").append(ulist);
                                    }
                                }
                                $("#userList>ul").find("li.dbl").addClass("pick").removeClass("dbl");
                            }
                        }) 

                    });
                    $("#addAllRolUser").off("click").on("click",function(){
                        var id=$(this).attr("data-nid");
                        $.fetch({
                            url: "/gateway/user/list",
                            type: 'post',
                    ...
                    $("#addAllPageUser").off("click").on("click",function(){
                    ...
                    $("#removerAllUser").off("click").on("click",function(){
                        $("#pickUserlist>ul").html('');
                        roleList=[];
                        $("#userList>ul").find("li.pick").addClass("dbl").removeClass("pick");
                    })
                }else{
                    $("#btmChosBtn").hide();
                    $("#userList>.deparTit").text('成员选择列表');
                    if(rs.data_list.length==0){
                        $("#userList>ul").html('<div style="height: 255px;line-height: 255px;text-align: center;font-size: 22px;color: #ccc;">查询不到此用户<div>');
    ...
    var ceckMenber =function(obj,page,o){//选中用户的方法
        var html ='';
        ...
        for(var i=0;i<obj.length;i++){
            ...
            html+='<li class="'+isPick+'" data-user='+obj[i].id+' data-titleName='+obj[i].title_name+' data-department='+obj[i].department_name+'>'+obj[i].name+' ('+obj[i].title_name+')</li>'
        }      
        $("#userList>ul").html(html);
        $("#userPage").show();
        var pageHtml='';
        pageHtml += '<p class="nowPage">第'+page.page_index+'页(共'+page.last_page+'页)</p><p class="prevPage">上一页</p><p class="nextPage">下一页</p>'
        ...
        //选中
        $("#userList>ul").off("click").on("click","li.dbl",function(){
            var roId=$(this).attr("data-user");
            $(this).addClass("pick").removeClass("dbl");
            //把选中的id 存入数组
            if(roleList.indexOf(roId) == -1){//判断数组是否存在当前id
                roleList.push(roId);
    ...
    $("#pickUserlist>ul").off("mouseenter").on("mouseenter","li",function(){
        $(this).append('<i class="iconfont icon-guanbi"></i>');
        $(this).children("i").one("click",function(){
            var userId = $(this).parent("li.pick").attr("data-user");
            roleList.splice($.inArray(userId,roleList),1);
            $(this).parent("li.pick").remove();
            $("#userList").find('li[data-user="'+userId+'"]').addClass("dbl").removeClass("pick");
        })
    });
    $("#pickUserlist>ul").off("mouseleave").on("mouseleave","li",function(){
    ...
    $("#main1 .browserTab").on("click","a",function(){
        ...
        $("#main1>ul").eq(index).show()
        $("#main1>ul").eq(index).siblings("ul").hide();

    })
}
var selDeparFun=function(el){//添加部门弹窗
    layTit=el.parent().attr('data-title');
    if(layTit == ''){
        layTit=' ';
    }
    ...
    var selTerr =function(o,el,ls){//生成列表的方法
        ...
        function forTree(o){
            for(var i =0;i<o.length;i++){
                html+='<li>'
                if(o[i].children&&o[i].children.length>0){
                    html+='<span class="dbl isChid folder" data-id="'+o[i].id+'"><i class="iconfont bclick icon-40"></i><i class="iconfont icon-zy_qunzuduoren"></i><a data-id="'+o[i].id+'">'+o[i].name+'</a></span><ul>'
                    forTree(o[i].children);//存在子级再次调用循环
                    html+="</ul>"
                }else{
                    html+='<span class="dbl noChid Shrink file" data-id="'+o[i].id+'"><i class="iconfont null"></i><i class="iconfont icon-zy_qunzuduoren"></i><a data-id="'+o[i].id+'">'+o[i].name+'</a></span>'
                }
        ...
        for(var i=0;i<roleList.length;i++){
            $('a[data-id="'+roleList[i]+'"]').parent("span").removeClass("dbl").addClass("layui-disabled");
        ...
        el.off("click").on("click","span.dbl>a",function(){  
            ...
            roleList.push(''+Id);
            $(this).parent("span").addClass("layui-disabled").removeClass("dbl");
    ...
    $("#pickUserlist>ul").off("mouseenter").on("mouseenter","li",function(){
        ...
        $(this).children("i").one("click",function(){
            ...
            roleList.splice($.inArray(userId,roleList),1);
            $(this).parent("li.pick").remove();
            $("#main1").find('a[data-id="'+userId+'"]').parent("span").removeClass("layui-disabled").addClass("dbl");
        })
    });
    $("#pickUserlist>ul").off("mouseleave").on("mouseleave","li",function(){
        $(this).children("i").remove();
...
var selRoleFun=function(el){//添加角色弹窗
    layTit=el.parent().attr('data-title');
    if(layTit == ''){
        layTit=' ';
    }
...
$.managerFun = function(obj={}){// 
    ...
    $.fetch({
        ...
        cb:function(rs){
            ...
            var rs =rs.userList;
            if(rs.length==0){
                $("#nextDacApp").parents('.layui-form-item').hide();
                rs= [{name: "暂无下一步", id: 0}];
            }else{
                $("#nextDacApp").parent('td').parent('tr').show();
            }
            obj.fun(rs,forw,steps);
        }
    })
}
$.pageFun=function(o={}){//自定义分页
    var page=o.page;
    var elem=o.elem;
    var pageLi='';
    ...
    elem.children(".pag").off("click").on('click',"li",function(){
        var n=$(this).text();
        if(index!=n){
            getList(n,o)
        }
...
var NumberCheck =function(num) {
  ...
  $.diffTime=function(starT,endT){//计算时间差
...
//
function toNonExponential(num) {//科学计数法转数字
    // console.log(num)
