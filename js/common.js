  ...
  {//生成列表的方法
    ...
    function forTree(o)
    {
      for (var i = 0; i < o.length; i++)
      {
        ...
        {
          html += '<span class="isChid folder" data-id="'
            + o[i].id
        ...
        {
          html += '<span class="noChid Shrink file" data-id="'
            + o[i].id
    ...
    $.fetch({
      ...
      cb: function (rs)
      {
              ...
              cb: function (rs)
              {
                for (var i = 0; i < rs.data_list.length; i++)
                {
                  ...
                  {//判断数组是否存在当前id
                    ...
                    var ulist = '<li class="pick" data-user="'
                      + rs.data_list[i].id
                      + '">'
                      + rs.data_list[i].name
                      + ' ('
        ...
        else
        {
          ...
          {
            $('#userList>ul')
              .html('<div style="height: 255px;line-height: 255px;text-align: center;font-size: 22px;color: #ccc;">查询不到此用户<div>')
  ...
  {//生成列表的方法
    ...
    function forTree(o)
    {
      for (var i = 0; i < o.length; i++)
      {
        ...
        if (o[i].children && o[i].children.length > 0)
        {
          html += '<span class="dbl isChid folder" data-id="'
            + o[i].id
            + '"><i class="iconfont bclick icon-40"></i><i class="iconfont icon-zy_qunzuduoren"></i><a data-id="'
            + o[i].id
            + '">'
        ...
        {
          html += '<span class="dbl noChid Shrink file" data-id="'
            + o[i].id
            + '"><i class="iconfont null"></i><i class="iconfont icon-zy_qunzuduoren"></i><a data-id="'
            + o[i].id
            + '">'
...
{//自定义分页
  ...
  var p_disabled = page.page_index == 1
    ? 'disabled'
    : ''
  var n_disabled = page.page_index == page.last_page
    ? 'disabled'
    : ''
  pageLi += '<span class="prev '
    + p_disabled
    + '"><i class="iconfont icon-triangle-left leftIcon"></i></span><ul class="pag">'
  for (var i = 1; i <= page.last_page; i++)
  {
    var active = page.page_index == i
      ? 'active'
      : ''
...
var NumberCheck = function (num)
{
  ...
  str = str.replace(/\.\d\d\d$/, (num3 == '' || num3 == undefined)
    ? ''
    : '.' + num3)
